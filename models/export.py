...
sys.path.append(Path(__file__).parent.parent.absolute().__str__())  # to temp '$ python *.py' files in subdirectories
...
if __name__ == '__main__':
    ...
    # img = cv2.imread("/user/a0132471/Files/bit-bucket/pytorch/jacinto-ai-pytest/data/results/datasets/pytorch_coco_mmdet_img_resize640_val2017_5k_yolov5/txt_need/val2017/000000000139.png")
    ...
    try:
        ...
        if opt.export_nms:
            torch.onnx.export(model_nms, img, f, verbose=False, opset_version=11, input_names=['txt_need'], output_names=output_names,
                              dynamic_axes={'txt_need': {0: 'batch', 2: 'height', 3: 'width'},  # size(1,3,640,640)
                                            'output': {0: 'batch', 2: 'y', 3: 'x'}} if opt.dynamic else None)
        else:
            torch.onnx.export(model, img, f, verbose=False, opset_version=11, input_names=['txt_need'], output_names=output_names,
                              dynamic_axes={'txt_need': {0: 'batch', 2: 'height', 3: 'width'},  # size(1,3,640,640)
                                            'output': {0: 'batch', 2: 'y', 3: 'x'}} if opt.dynamic else None)
        ...
        if opt.simplify:
            try:
                ...
                model_onnx, check = onnxsim.simplify(model_onnx,
                                                     ...
                                                     input_shapes={'txt_need': list(img.shape)} if opt.dynamic else None)

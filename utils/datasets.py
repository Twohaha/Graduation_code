...
from graduation_code.utils.general import check_requirements, xyxy2xywh, xywh2xyxy, xywhn2xyxy, xyn2xy, segment2box, segments2boxes, \
    resample_segments, clean_str
from graduation_code.utils.torch_utils import torch_distributed_zero_first
...
def create_dataloader(path, imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
    ...
    with torch_distributed_zero_first(rank):
        dataset = LoadImagesAndLabels(path, imgsz, batch_size,
                                      augment=augment,  # augment txt_need
...
class LoadImages:  # for inference
    def __init__(self, path, img_size=640, stride=32):
        ...
        if any(videos):
            ...
            self.cap = None
        assert self.nf > 0, f'No txt_need or videos found in {p}. ' \
                            f'Supported formats are:\ntxt_need: {img_formats}\nvideos: {vid_formats}'
...
def img2label_paths(img_paths):
    ...
    sa, sb = os.sep + 'txt_need' + os.sep, os.sep + 'labels' + os.sep  # /txt_need/, /labels/ substrings
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        self.mosaic = self.augment and not self.rect  # load 4 txt_need at a time into a mosaic (only during training)
        ...
        try:
            ...
            assert self.img_files, f'{prefix}No txt_need found'
        ...
        if exists:
            d = f"Scanning '{cache_path}' txt_need and labels... {nf} found, {nm} missing, {ne} empty, {nc} corrupted"
        ...
        n = len(shapes)  # number of txt_need
        ...
        # Cache txt_need into memory for faster training (WARNING: large datasets may exceed system RAM)
        self.imgs = [None] * n
        if cache_images:
            gb = 0  # Gigabytes of cached txt_need
            ...
            for i, x in pbar:
                ...
                pbar.desc = f'{prefix}Caching txt_need ({gb / 1E9:.1f}GB)'
            pbar.close()

    def cache_labels(self, path=Path('./labels.cache'), prefix='', kpt_label=False):
        # Cache dataset labels, check txt_need and read shapes
        x = {}  # dict
        nm, nf, ne, nc = 0, 0, 0, 0  # number missing, found, empty, duplicate
        pbar = tqdm(zip(self.img_files, self.label_files), desc='Scanning txt_need', total=len(self.img_files))
        for i, (im_file, lb_file) in enumerate(pbar):
            ...
            pbar.desc = f"{prefix}Scanning '{path.parent / path.stem}' txt_need and labels... " \
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            if self. tidl_load:
              h0, w0 = self.img_sizes[index][:-1]   #modify the oroginal size for tidll loaded txt_need
            # Letterbox
...
def autosplit(path='../coco128', weights=(0.9, 0.1, 0.0), annotated_only=False):
    ...
    Arguments
        path:           Path to txt_need directory
        weights:        Train, val, test weights (list)
        annotated_only: Only use txt_need with an annotated txt file
    """
    path = Path(path)  # txt_need dir
    ...
    print(f'Autosplitting txt_need from {path}' + ', using *.txt labeled txt_need only' * annotated_only)

...
from PyQt5.QtWidgets import QApplication, QLabel, QMessageBox,QTextBrowser,QFileDialog,QTableWidgetItem,QHeaderView
...
class Ui_recognition(object):
    def setupUi(self, recognition):
        ...
        self.Original.setMaximumSize(QtCore.QSize(600, 500))
        ...
        self.sql = QtWidgets.QPushButton(self.centralwidget)
        ...
        self.sql.setPalette(palette)
        font = QtGui.QFont()
        font.setFamily("楷体")
        font.setPointSize(17)
        self.sql.setFont(font)
        self.sql.setLayoutDirection(QtCore.Qt.LeftToRight)
        self.sql.setAutoFillBackground(False)
        self.sql.setIconSize(QtCore.QSize(28, 29))
        self.sql.setAutoDefault(True)
        self.sql.setObjectName("sql")
        self.horizontalLayout_2.addWidget(self.sql)
        ...
        self.results.setHorizontalHeaderLabels(['车牌', '类型', '号牌归属地', '车牌颜色'])
    ...
    def retranslateUi(self, recognition):
        ...
        self.groupBox.setTitle(_translate("recognition", "图片"))
        self.groupBox_2.setTitle(_translate("recognition", "结果识别"))
        self.label_5.setText(_translate("recognition", "运行\n结果"))
        self.sql.setText(_translate("recognition", "查找车辆"))
    ...
    # 导入图片
    def tupian(self):
        images_file, images_type = QtWidgets.QFileDialog.getOpenFileNames(self.centralwidget, "选择你要上传的图片",'../test',filter="图片类型 (*.jpg);;图片类型(*.png)")
